...
- **Stars & Stripes action cards:** official Restoration/IELLO rules and component pages are first-party for set/fighter mechanics and quantities, while complete action-card text is still primarily a transcription layer from current physical/card-image indexes where first-party machine-readable card text is unavailable. The manifests keep this provenance distinction explicit; no `/decks/...` fan preview or balance-patch values are accepted as official data.
