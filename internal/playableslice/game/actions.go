...
func (m *Match) playScheme(playerID string, command Command) error {
	...
	case content.EffectHorns:
		...
		target := m.Fighters[command.TargetID]
		if target == nil || target.Defeated || target.ID == fighter.ID {
			return reject("illegal_target", "Jackalope Horns requires another living adjacent fighter")
		}
		if !m.adjacent(finalSpace, target.SpaceID) {
			return reject("illegal_target", "target is not adjacent to Jackalope's destination")
		}
	default:
	...
	case content.EffectHorns:
		m.move(fighter, command.Path)
		m.damage(command.TargetID, 2, "scheme")
	}
	if len(m.Pending) == 0 {
		m.finishAction()
	}
